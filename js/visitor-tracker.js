...
(function (global) {
  ...
  function drawMap(rows) {
    ...
    // Dot color is themeable via VISITOR_CONFIG.DOT_COLOR (hex), so the map
    // can match the site accent. Defaults to the original purple.
    var dot = CFG.DOT_COLOR || "#6600ff";
    var m = /^#?([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})$/i.exec(dot);
    var rgb = m ? parseInt(m[1], 16) + "," + parseInt(m[2], 16) + "," + parseInt(m[3], 16) : "102,0,255";
    ...
    // Abstract halftone backdrop: rasterize land polygons from a GeoJSON
    // (equirectangular, same projection as the visitor dots) offscreen, then
    // stipple a staggered dot grid over the land pixels. Themeable via
    // MAP_LAND_COLOR / MAP_DOT_STEP so it can sit on any site palette.
    function drawDotBackdrop() {
      return fetch(CFG.MAP_GEOJSON)
        .then(function (r) { return r.json(); })
        .then(function (geo) {
          ...
          var land = octx.getImageData(0, 0, W, H).data;
          var step = CFG.MAP_DOT_STEP || 10;
          var rad = CFG.MAP_DOT_RADIUS || step * 0.27;
          ctx.fillStyle = CFG.MAP_LAND_COLOR || "rgba(0,0,0,0.16)";
          for (var y = step / 2, row = 0; y < H; y += step, row++) {
            for (var x = step / 2 + (row % 2) * step / 2; x < W; x += step) {
              if (land[((y | 0) * W + (x | 0)) * 4 + 3] > 128) {
    ...
    function drawImageBackdrop() {
      ...
      img.onload = function () {
        // Optional CSS-style filter (e.g. "grayscale(70%)") applied to the
        // backdrop only, so the dots stay crisp on themed pages.
        if (CFG.MAP_FILTER && "filter" in ctx) ctx.filter = CFG.MAP_FILTER;
      ...
      img.onerror = function () {
        ...
        ctx.fillStyle = CFG.MAP_FALLBACK_BG || "#efe9e0";
    ...
    if (CFG.MAP_STYLE === "dots" && CFG.MAP_GEOJSON) {
      drawDotBackdrop().catch(drawImageBackdrop);
    } else {
      drawImageBackdrop();
    }
  }
